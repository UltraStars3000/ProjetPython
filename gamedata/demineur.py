...
from time import *
from gamedata.afficheur import Afficheur
from gamedata.scores import Scores
from gamedata.libs.playsound import playsound
import os
import sys
...
class Demineur():
    ...
    def __init__(self, parent, nombreColonnes=5, nombreLignes=10, forcageNombreMines=None,
        ...
        if self.marathon != None:
            ...
            self.root.title("Demineur " + str(self.NB_COLS) + "x" + str(self.NB_LINES))
        self.root.protocol("WM_DELETE_WINDOW", self.onClosing)
        #Chargement des images:
        self.mine = PhotoImage(file="gamedata/img/mine.gif")
        self.mine_explosee = PhotoImage(file="gamedata/img/mine_explosee.gif")
        self.drapeau = PhotoImage(file="gamedata/img/drapeau.gif")
        self.precedent = PhotoImage(file="gamedata/img/precedent.gif")
        self.smiley_content = PhotoImage(file="gamedata/img/smiley_content.gif")
        self.smiley_mort = PhotoImage(file="gamedata/img/smiley_mort.gif")
        self.aide = PhotoImage(file="gamedata/img/aide.gif")
        ...
        Button(self.root, text="Menu principal", bd=0, font=("Times New Roman", 10, "bold"), bg='lightgray', activebackground='black', fg='black', activeforeground='lightgray', width=15, command=self.menuPrincipal).grid(row=2, column=1)
        self.grille=self.generationGrilleVide()
        self.refreshScreen()
        self.root.mainloop()
    
    def menuPrincipal(self):
        if self.scoreboardActif == False:
    ...
    def revelerGrille(self, y, x):
        ...
        if not(self.mute):
            playsound("gamedata/sounds/explosion.mp3", block = False)
        for i in range(len(self.grille[1])):
    ...
    def actualiserTimer(self):
        ...
        if not(self.reset):
            ...
            if not(self.win) and not(self.perdu) and (self.marathon == None):
                ...
                if not(self.mute):
                    playsound("gamedata/sounds/ta_da.mp3", block = False)
                print("Partie gagné !")
                if self.marathon != None:
    ...
    def finPartie(self):
        ...
        self.scoreboard.protocol("WM_DELETE_WINDOW", self.cancelScoreboard)
        self.scoreboardActif = True
